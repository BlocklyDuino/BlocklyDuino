...
function load(event) {
  ...
  reader.onloadend = function(event) {
    ...
    if (target.readyState == 2) {
      ...
      if (count && confirm('Reemplazar bloques existentes?\nPulsar "Cancelar" para cargar los nuevos bloques conservando los existentes.')) {
...
function discard() {
  var count = Blockly.mainWorkspace.getAllBlocks().length;
  if (count < 2 || window.confirm('Borrar los ' + count + ' bloques?')) {
    Blockly.mainWorkspace.clear();
    renderContent();
  }
}
...
function load_by_url(uri) {
  ajax = createAJAX();
  if (!ajax) {
　　   alert ('No compatible con XMLHttpRequest');
　　   return 0;
　  }
